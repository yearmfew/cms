...
class Portfolio extends CI_Controller
{
    ...
    public function save(){
        ...
        if ( $this->input->post("category_id") === "addNew"   ) {
         $this->form_validation->set_rules('new_category', 'Yeni Kategori', 'trim|required');
     ...
     if($validate){
       ...
       if ( $this->input->post("category_id") === "addNew"   ){
           ...
           $insert_category = $this->portfolio_category_model->add(
            array(
                "title" => $this->input->post("new_category"),
                "isActive"      => 1,
           ...
           $new_item = $this->portfolio_category_model->get(array(
            "title" => $this->input->post("new_category")
        )
       );

           $category_id = $new_item->id;
...
public function imageDelete($id, $parent_id){
    ...
    if($delete){
        ...
        unlink("uploads/{$this->viewFolder}/255x156/$fileName->img_url");
...
public function image_upload($id){
    ...
    $image_255x156  = upload_picture($_FILES["file"]["tmp_name"], "uploads/$this->viewFolder",255,156, $file_name);
    $image_1080x426 = upload_picture($_FILES["file"]["tmp_name"], "uploads/$this->viewFolder",1080,426, $file_name);

    if($image_255x156 && $image_1080x426){
