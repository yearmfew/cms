...
class Send_email extends CI_Controller
{
    ...
    public function index(){
        ...
        $items = $this->sendEmail_model->get_all(
            array()
        );
    ...
    public function save(){
        ...
        if($validate){
            ...
            $insert = $this->sendEmail_model->add(
                array(
    ...
    public function send(){
        ...
        $this->form_validation->set_rules('to', 'Gönderilecek Mail', 'required|valid_email');
        $this->form_validation->set_rules('subject', 'Başlık', 'required');
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $send = send_email($to, $subject, $message);
            // TODO Alert sistemi eklenecek...
            if($send){

                $alert = array(
                    "title" => "Tebrikler",
                    "text" => "E-postanız başarılı bir şekilde gönderildi...",
                    "type"  => "success"
                );
                ...
                $alert = array(
                    "title" => "Uppps",
                    "text" => "Mesaj gitmedi nasip değilmiş...",
                    "type"  => "error"
                );
            }
...
public function update($id){
    ...
    if($validate){
            ...
            // Upload Süreci...
        $update = $this->sendEmail_model->update(
            array("id" => $id),
            array(
                "protocol"      => $this->input->post("protocol"),
                "host"          => $this->input->post("host"),
        ...
        /** Tablodan Verilerin Getirilmesi.. */
        $viewData->item = $this->sendEmail_model->get(
            array(
                "id"    => $id,
            )
        );
...
public function delete($id){
    ...
    $delete = $this->sendEmail_model->delete(
        array(
            "id"    => $id
        )
    );
